...
### Package Naming
- Root: `com.stano.springapp`
- Second level: entity aggregate name (e.g., `person`, `order`)
- Third level (optional): layer/concern (e.g., `api`, `service`, `repository`)

Examples:
- `com.stano.springapp.person.api` — REST controller for Person
- `com.stano.springapp.person` — domain entity and value objects
- `com.stano.springapp.person.service` — application service
